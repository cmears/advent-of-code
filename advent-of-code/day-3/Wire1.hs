...
path (x,y) [] = [(x,y)]
path (x,y) ((_,0):moves) = path (x,y) moves
path (x,y) ((d,n):moves) = (x,y) : path (x',y') ((d,n-1):moves)
  where (x',y') = case d of
                    'U' -> (x,y+1)
                    'D' -> (x,y-1)
                    'R' -> (x+1,y)
